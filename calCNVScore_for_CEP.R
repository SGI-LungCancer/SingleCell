calCNVScore <- function(sh, cell_info, s, levels, cutoff.corr, cutoff.score, meta){
  ...
  cell_info2 <- plyr::join(cell_info, CNV_score, by="Row.names") # boxplot for cluster
  cell_info2$cluster <- factor(cell_info2$cluster, levels = levels)

  ## Boxplot per each cluster - MS
  ggplot(cell_info2,aes(x=cluster ,y=MS, fill=cluster))  +
  ...
  tumorcells <- filter(cell_info3, ((MS > cutoff.score | COR > cutoff.corr) & cluster == "EP"))$Row.names
  nontumorcells <- cell_info3$Row.names[!(cell_info3$Row.names %in% c(tumorcells))]
  immunecells <- cell_info3$Row.names[cell_info3$cluster != "EP"]
  ...
  expos<-ggplot(cell_info3, aes(x=MS, y= COR)) + geom_point(aes(fill=cell_index), size=5, alpha=.8, shape=21, colour="black") +
    ...
    theme(panel.border=element_rect(fill=NA, colour="black", size=2), legend.position = 'none')
  expos
  ggsave(paste0("final_",s, "_all_cells_CNV_score_vs_cor_classification.pdf"), width = 5, height = 5)

  ## Tumor vs. Nontumor frequency ##
  tb <- cell_info3 %>% dplyr::group_by(cluster, cell_index) %>% dplyr::summarise(n = n())
  write.table(tb, paste0("Final_QC_Celltype_info_table_", s, ".txt"), sep = "\t", quote = F, row.names = T)
  write.table(cell_info3, paste0("Final_QC_Celltype_info_", s, ".txt"), sep = "\t", quote = F, row.names = T)
